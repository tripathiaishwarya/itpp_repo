...
int itpp_crc_decode(char *fname, unsigned long fname_len)
{
	...
	double *matrixOfCrcDecodedBits = NULL;
	char **str = NULL;
	int *strlen = NULL;
	int *flag = NULL;		
	...
	inputVector = itpp::to_bvec(vec(matrixOfInputBits, m1*n1)); 


	//Invoking the Hamming code encoding function
	outputFlag = CRC_Code(*str).decode(inputVector,outputVector);
	

	//----------------------------------------Output Arguements---------------------------------------

	//If parity check is True
	if(outputFlag == TRUE)
	{
		/////////////////////////////////////////////////////////////////////////////////////
		//Storing the output in a double array to be sent to scilab
		matrixOfCrcDecodedBits = (double *) malloc( sizeof(double)*outputVector.length());
		for( i=0; i<outputVector.length(); i++ )
			matrixOfCrcDecodedBits[i] = outputVector(i);		
		...
		sciErr = createMatrixOfDouble(pvApiCtx, nbInputArgument(pvApiCtx) + 1, row, col, matrixOfCrcDecodedBits);	 
    		if (sciErr.iErr)
    		{
        		printError(&sciErr, 0);
        		return 0;
    		}
		free(matrixOfCrcDecodedBits); 
		///////////////////////////////////////////////////////////////////////////////////////
	}
	
	
	//If parity check is not OK
	else
	{
		///////////////////////////////////////////////////////////////////////////////////////
		Scierror(999, _("%s: The Parity Check is not OK.Recheck the input provided.\n"), fname, 1);
		return 0;
		///////////////////////////////////////////////////////////////////////////////////////
	}
