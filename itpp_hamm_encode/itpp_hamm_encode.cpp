/*----------------------------------------------------------------------------------------------------------------
Code for Hamming Code Encoding 
...
*-----------------------------------
* The input to the encode() function is a binary vector and an integer M, 
...
int itpp_hamm_encode(char *fname, unsigned long fname_len)
{
	...
	double *matrixOfHammEncodedBits = NULL;
	double *m;
	...
	//---------------------------Implementation of Hamming Code Encoding------------------------------
	...
	outputVector = Hamming_Code((int) *m).encode(inputVector);
	

	//----------------------------------------Output Arguements---------------------------------------
	//Storing the output in a double array to be sent to scilab
	matrixOfHammEncodedBits = (double *) malloc( sizeof(double)*outputVector.length());
	for( i=0; i<outputVector.length(); i++ )
		matrixOfHammEncodedBits[i] = outputVector(i);		
	...
	sciErr = createMatrixOfDouble(pvApiCtx, nbInputArgument(pvApiCtx) + 1, row, col, matrixOfHammEncodedBits);	 
    	if (sciErr.iErr)
    	{
        	printError(&sciErr, 0);
        	return 0;
    	}
	free(matrixOfHammEncodedBits); 
